  ...
  return (
    <div className="dial-wrap" onClick={onClose}>
      <div className="dial" onClick={(e) => e.stopPropagation()}>
        ...
        {!calling ? (
          <button className="dial-call" disabled={!step.contactPhone} onClick={async () => {
            setCalling(true);
            // Place a real Zoom call when connected; the timer reflects the live call.
            try {
              const r = await zoomCallAction(step.contactPhone);
              if (r.placed) showToast("Zoom is ringing your phone — pick up to connect");
              else if (r.error === "not connected") showToast("Connect Zoom in Coaching to auto-dial");
            } catch { /* keep the manual timer */ }
          }}>
