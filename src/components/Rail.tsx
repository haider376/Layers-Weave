...
export type RailRow = { label: string; value: string; sub?: string; pct?: number; rank?: number };
    ...
    <div className="rail-lb">
      {rows.map((r, i) => (
        <div className="rail-lrow" key={r.label + i}>
          {avatars ? <span className="mini-av">{initials(r.label)}</span> : <span className="rail-rank">{r.rank ?? i + 1}</span>}
